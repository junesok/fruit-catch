...
let baskets = [
  { x: 0.5,  y: 0.8, targetX: 0.5,  targetY: 0.8, width: 0, height: 0 },
  { x: 0.75, y: 0.8, targetX: 0.75, targetY: 0.8, width: 0, height: 0 },
];
  ...
  if (!tracker) {
    ...
    tracker.onHandMove = (hands) => {
      if (gameMode === '1p') {
        ...
        if (h) { baskets[0].targetX = h.x; baskets[0].targetY = h.y; }
      } else {
        if (hands[0]) { baskets[0].targetX = hands[0].x; baskets[0].targetY = hands[0].y; }
        if (hands[1]) { baskets[1].targetX = hands[1].x; baskets[1].targetY = hands[1].y; }
      }
    };
...
function startGame() {
  ...
  baskets[0].targetX = 0.25; baskets[0].targetY = 0.8;
  baskets[1].x = 0.75; baskets[1].y = 0.8;
  baskets[1].targetX = 0.75; baskets[1].targetY = 0.8;
...
function update() {
  ...
  for (let i = fruits.length - 1; i >= 0; i--) {
    ...
    for (const b of checkBaskets) {
      const bx     = b.x * W;
      const margin = b.height * .6;
      const by     = Math.min(Math.max(b.y * H, margin), H - margin);
      const bHalfW = b.width * .48;
      const bTop   = by - b.height * .3;

      const dx = Math.abs(f.x - bx);
      const dy = Math.abs(f.y - bTop);

      if (dx < bHalfW + f.size * .3 && dy < b.height * .55) {
        ...
        if (f.isBomb) {
          ...
          catchBreakdowns[pIdx][f.fruit.label] = (catchBreakdowns[pIdx][f.fruit.label] || 0) + 1;
...
function draw() {
  ...
  activeBaskets.forEach(b => {
    ...
    const by     = Math.min(Math.max(b.y * H, margin), H - margin);
    ctx.save();
    ctx.font = `${b.width * .8}px serif`;
    ctx.textAlign    = 'center';
    ctx.textBaseline = 'middle';
    ctx.globalAlpha  = 1;
    ctx.fillText(BASKET_EMOJI, bx, by);
    ctx.restore();
  });

  // 손 위치 가이드
  if (tracker) {
    tracker.lastHands.forEach((h, idx) => {
      if (!h) return;
      if (gameMode === '1p' && idx > 0) return;
      const hx = h.x * W;
      const hy = Math.min(Math.max(h.y * H, baskets[0].height * .6), H - baskets[0].height * .6);
      ctx.save();
      ctx.beginPath();
